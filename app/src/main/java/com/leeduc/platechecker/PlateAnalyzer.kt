 ...
 * sau đó xác định màu nền biển số dựa trên vùng ảnh bao quanh vùng chữ nhận diện
 * được, rồi ghép hậu tố màu vào cuối chuỗi ký tự.
 *
 * Quy ước hậu tố:
 *   Vàng       -> V
 *   Xanh dương -> X
 *   Trắng      -> T
 *   Đỏ         -> (không có hậu tố, để nguyên chuỗi ký tự)
 *
 * Cách phân loại: dùng HSV thay vì so trực tiếp kênh RGB. "Độ đậm màu" (saturation,
 * 0..1) phải đạt tối thiểu một ngưỡng % nhất định mới được công nhận là màu đó,
 * nếu không sẽ mặc định về trắng - tránh trường hợp biển trắng bị ánh đèn flash
 * làm ngả nhẹ sang xanh/vàng mà vẫn bị báo nhầm màu.
 */
object PlateAnalyzer {

    /**
     * Ngưỡng độ đậm màu (saturation) tối thiểu để công nhận 1 màu, tính theo %
     * (0.5f = 50%). Tăng lên nếu vẫn còn báo nhầm màu quá dễ dàng; giảm xuống
     * nếu biển đúng màu nhưng bị báo thành trắng.
     */
    private const val COLOR_SATURATION_THRESHOLD = 0.50f
     ...
     * thay vì chỉ chữ đen), rồi phân loại theo HSV. Trả về "V"/"X"/"T", hoặc chuỗi
     * rỗng nếu là biển đỏ (không cần hậu tố).
     */
    private fun classifyPlateColor(bitmap: Bitmap, textRect: Rect): String {
        ...
        val hue = hsv[0]        // 0-360
        val saturation = hsv[1] // 0-1, "độ đậm màu" - càng gần 1 càng rực, gần 0 càng xám/trắng
        val value = hsv[2]      // 0-1, độ sáng

        val hasEnoughColor = saturation >= COLOR_SATURATION_THRESHOLD

        return when {
            // Trắng: sáng và gần như không có màu -> ưu tiên xét trước
            value > 0.6f && saturation < 0.25f -> "T"
            // Đỏ: hue nằm quanh mốc 0 độ (đỏ có thể là 0-15 hoặc 345-360 tuỳ vòng tròn màu)
            hasEnoughColor && (hue <= 15f || hue >= 345f) -> "" // không thêm hậu tố
            // Vàng: hue khoảng 30-75 độ
            hasEnoughColor && hue in 30f..75f -> "V"
            // Xanh dương: hue khoảng 190-250 độ
            hasEnoughColor && hue in 190f..250f -> "X"
            // Không màu nào đủ đậm/đúng khoảng hue -> mặc định về trắng (an toàn hơn đoán bừa)
            else -> "T"
        }
    }
}
